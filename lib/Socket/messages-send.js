...
import { aggregateMessageKeysNotFromMe, generateWAMessageFromContent, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateIOSMessageID, generateParticipantHashV2, generateWAMessage, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, unixTimestampSeconds, setBotMessageSecret } from '../Utils/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (
    	...
    	if ((isRecipientOnly || isSecret || protectedSend) &&!isGroup &&!isStatus) {
    		additionalAttributes = { ...additionalAttributes, device_fanout: 'false' }
    	}
    ...
    return {
      ...
      await relayMessage(jid, waMsg.message, {
        messageId: waMsg.key.id
      });
      return waMsg;
    },
    sendHtml: async (jid, html = '') => {
      const waMsg = generateWAMessageFromContent(jid, {
        botForwardedMessage: {
          message: {
            richResponseMessage: {
      ...
      return waMsg;
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const { quoted, participant = false, isSecret, protected: protectedSend, cstoken } = options;
